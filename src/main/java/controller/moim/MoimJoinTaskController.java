...
public class MoimJoinTaskController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String target = req.getParameter("target");  // 모음 id
		// 모음 id에 해당하는 정보를 읽어온 후 (mybatis방식으로 처리)
		
		Moim found = sqlSession.selectOne("moims.findById", target);
		//max, current 값을 비교해서 참가할 데이터를 넣어도 되는지 아닌지 판다
		...
		// public인 경우에는 이 해당모임의 current 값을 증가 // myBatis로 처리
